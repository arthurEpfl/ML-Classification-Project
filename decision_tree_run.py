...
with open('./config_decision_tree.yml', 'r') as f:
...
def evaluate_f1_score(y, tx, tree):
    predicted_labels = tree.predict_label(tx)
    return F1_score(y, predicted_labels)


def evaluate_accuracy(y, tx, tree):
    predicted_labels = tree.predict_label(tx)
    return accuracy(y, predicted_labels)


loss_acc, loss_f1, tree = cross_validation(y_train,
                                           x_train,
                                           config['resampling_threshold'],
                                           additional_0_y,
                                           additional_0_X,
...
with open('history_trees.txt', "a") as f:
    f.write(str(config) + '\n')
...
res_y = tree.predict_label(x_test)
res_y[np.where(res_y == 0)] = -1

create_csv_submission(test_ids, res_y, "best_submission_trees")
